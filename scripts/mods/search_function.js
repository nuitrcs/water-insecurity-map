...
// d3.select("#fourth_bar").call(d3.slider().scale(d3.time.scale().domain([new Date(2017,5,31), new Date(2018,5,1)])).axis(d3.svg.axis()).on("slide", function(evt, value){
//     d3.select('#year_value2').text(value);
//     filter_list(clicked =jun.clicked)
//     center_changer()
// }))

// d3.select("#fourth_bar").call(d3.slider().scale(d3.time.scale().domain([new Date(2017,5,31), new Date(2018,5,1)])).axis(d3.svg.axis()))
